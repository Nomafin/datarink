				...
				lines.forEach(function(l) {
					...
					prds.forEach(function(prd) {
						...
						if (fd === "f" && linemateRows.length === 3) {
							playerIntersection = intersect.big(linemateRows[0].shifts, linemateRows[1].shifts, linemateRows[2].shifts);
						} else if (fd === "d" && linemateRows.length === 2) {
							playerIntersection = intersect.big(linemateRows[0].shifts, linemateRows[1].shifts);
						}
						...
						if (playerIntersection) {
							...
							prdSsRows.forEach(function(sr) {
								lineObj[sr.strength_sit].toi += intersect.big(playerIntersection, sr.timeranges).length;
							});
						}
					});
				});
